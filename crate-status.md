...
### gix-pathspec
* [x] parse single
* [x] parse file line by line (with or without quoting, NUL and LF/CRLF line separation) (see `--pathspec-from-file` and `--pathspec-file-nul`)
...
* [x] special handling of submodules (for now, submodules or nested repositories are detected, but they can't be walked into naturally)
...
* [x] when reporting reachable objects, provide the path through which they are reachable, i.e. ref-log@{3} -> commit -> tree -> path-in-tree
...
### gix-ref
* [x] Prepare code for arrival of longer hashes like Sha256. It's part of the [V2 proposal][reftable-v2] but should work for loose refs as well.
* **Stores**
